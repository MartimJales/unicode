...
{
    // Variáveis referentes à stack;
    int top = -1;
    struct elemento stack[10];

    //Array com os operadores utilizados na função operador;
    char simbolos[9] = {'+', '-', '*', '/', '^', '|', '#', '%', '&'};
    ...
    for (char *token = strtok(line, delimitadores); token != NULL; token = strtok(NULL, delimitadores))
    {
        ...
        else
        {
            struct elemento x = POP(top, stack);
            struct elemento y = POP(top, stack);
            PUSH(top, stack, operador(x, y, token));
        }
        // printf("STACK:");
        // PRINT_STACK_DEBUG();
    }
...
void PRINT_STACK(int top, struct elemento stack[])
{
    char *resto;
    for (int i = 0; i <= top; i++)
    {
